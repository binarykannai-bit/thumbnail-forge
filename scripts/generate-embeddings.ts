...
async function main() {
  ...
  /**
   * Rate Limit (429) エラー時に指数バックオフでリトライする fetch ラッパー
   * - 最大 maxRetries 回までリトライ
   * - 1回目: 60秒、2回目: 90秒、3回目: 120秒 待機
   */
  async function fetchWithRetry(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      if (attempt === maxRetries) return resp; // 諦め

      // Rate Limit ヒット → 待機してリトライ
      const waitSec = 60 + attempt * 30;
      console.log(
        `  ⏳ Rate Limit (429) を検出。${waitSec}秒待機してリトライ... (${attempt + 1}/${maxRetries})`
      );
  ...
  for (let i = 0; i < toProcess.length; i++) {
    ...
    try {
      ...
      // 4-2. GPT-4o-mini で説明文を生成（Rate Limit リトライ付き）
      console.log('  🤖 説明文を生成中...');
      const descResp = await fetchWithRetry(
      ...
      // 4-3. Embedding 生成（Rate Limit リトライ付き）
      console.log('  🔢 ベクトル化中...');
      const embedResp = await fetchWithRetry(
