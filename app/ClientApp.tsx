...
export default function ClientApp() {
  ...
  const [data, setData] = useState({
    mainCategory: '', subCategory: '', audiences: [], psychology: [], persona: '', direction: [], mainVisual: '', density: '', fontSize: '', ng: [],
  ...
  // Call our /api/generate-image route which proxies to OpenAI Image 2.0
...
function LoadingOverlay({ phase, msg, progress, thumbStatuses }: any) {
  const phases = [
    ...
    { id: 'images', label: 'サムネ画像生成', desc: 'Image 2.0で5枚生成' },
  ];
  ...
  return (
    <div style={styles.loadingOverlay}>
      <div style={styles.loadingBox}>
        ...
        {/* Progress bar (only during image phase) */}
        {progress.total > 0 && (
          <>
        ...
        <div style={styles.loadingHint}>
          {phase === 'analyzing' && 'GPT-5が原稿から最大のフック・キラーワード・心理トリガーを抽出しています'}
          {phase === 'titles' && 'タイトル3案 + サムネ5案のプロンプトを構築中（30秒〜1分）'}
          {phase === 'images' && '各画像の生成に20〜40秒かかります。並列処理中...'}
        </div>
      </div>
    </div>
  );
...
function Step5({ result, setResult }: any) {
  ...
  return (
    <div style={styles.stepContent}>
      ...
      {result.promptOnlyMode && (
        <div style={styles.promptModeBanner}>
          <Code2 size={20} style={{ flexShrink: 0, color: '#FF0000' }} />
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 700, marginBottom: 2, fontSize: 14, color: '#0F0F0F' }}>プロンプト出力モード（テスト用）</div>
            <div style={{ fontSize: 12, color: '#606060', lineHeight: 1.6 }}>
              現在はArtifact環境のため、画像生成はChatGPTに手動で渡す方式になっています。
              各カードの「プロンプトをコピー」または「ChatGPTで画像生成」ボタンを使用してください。
              <strong style={{ color: '#FF0000' }}>本番化（Next.js + Vercel）後は完全自動化されます。</strong>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={styles.patternsHeader}>
        <div>
          <h3 style={styles.patternsTitle}>サムネ案</h3>
          <p style={styles.patternsDesc}>各パターン = サムネ文字案 + Image 2.0プロンプトのセット</p>
        </div>
      ...
      <div style={styles.usageBox}>
        <div style={styles.usageHeader}>
          <Play size={16} fill="#FF0000" stroke="#FF0000" />
          <span>使い方</span>
        </div>
        <ol style={styles.usageList}>
          <li>気に入ったサムネ案の「<strong>ChatGPTで画像生成</strong>」ボタンをクリック</li>
          <li>ChatGPTが新タブで開き、プロンプトが自動的にクリップボードにコピーされる</li>
          <li>ChatGPT (Plus/Pro) のチャット欄に貼り付けて画像生成</li>
          <li>必要に応じてサムネ文字案をCanva等で重ねて完成</li>
        </ol>
      </div>
    </div>
  );
...
function ThumbnailImageCard({ data, index }: any) {
  ...
  const copyAll = () => {
    const textPart = t.sub ? `${t.main}\n${t.sub}` : t.main;
    const combined = `【サムネ文字案】\n${textPart}\n\n【Image 2.0プロンプト】\n${data.imagePrompt || ''}`;
  ...
  const openChatGPT = () => {
    // Copy prompt then open ChatGPT
    navigator.clipboard.writeText(data.imagePrompt || '');
    window.open('https://chat.openai.com/', '_blank', 'noopener,noreferrer');
  };

  const renderWithMarker = (text, marker) => {
    ...
    <div style={styles.thumbCard}>
      ...
      <div style={styles.thumbCardHeader}>
        ...
        <button onClick={copyAll} style={{
          ...styles.copyAllBtn,
          ...(allCopied ? styles.copyBtnDone : {})
        }} className={allCopied ? '' : 'copy-all-hover'}>
          {allCopied ? <Check size={14} /> : <Copy size={14} />}
          <span>{allCopied ? 'コピー済' : '全部コピー'}</span>
        </button>
      </div>
      ...
      <div style={styles.patternBody}>
        ...
        <div style={styles.patternSection}>
          <div style={styles.patternSectionLabel}>
            <Code2 size={12} />
            <span>Image 2.0プロンプト</span>
            <button onClick={copyPrompt} style={{
              ...styles.miniCopyBtnRed,
              ...(promptCopied ? styles.miniCopyBtnDone : {})
            }} className={promptCopied ? '' : 'mini-copy-red-hover'}>
              {promptCopied ? <Check size={12} /> : <Copy size={12} />}
              <span>{promptCopied ? 'コピー済' : 'プロンプトをコピー'}</span>
            </button>
          </div>
          <div style={{ ...styles.imgPromptBody, maxHeight: expanded ? 'none' : 120 }}>
            {data.imagePrompt || '(プロンプトなし)'}
          </div>
          <button onClick={() => setExpanded(!expanded)} style={styles.expandBtn} className="expand-btn-hover">
            {expanded ? '折りたたむ' : 'すべて表示'}
          </button>
        </div>

        {/* Open ChatGPT button */}
        <button onClick={openChatGPT} style={styles.openChatGPTBtn} className="open-chatgpt-hover">
          <ExternalLink size={14} />
          <span>ChatGPTで画像生成（プロンプトを自動コピー）</span>
        </button>
      </div>
    </div>
  );
