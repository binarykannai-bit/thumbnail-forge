...
// MAIN APP
// ============================================================
export default function ClientApp({ userEmail }: { userEmail?: string | null } = {}) {
  ...
  return (
    <div style={styles.app}>
      ...
      <Header reset={reset} hasResult={!!result} userEmail={userEmail} />
...
// HEADER
// ============================================================
function Header({ reset, hasResult, userEmail }: any) {
  return (
    ...
    <div style={styles.stepContent}>
      ...
      <div style={styles.infoBox}>
        ...
        <div>
          ...
          <div style={{ fontSize: 13, color: '#606060', lineHeight: 1.7 }}>
            「サムネを生成」ボタンを押すと、GPT-5が<strong>タイトル案3つ</strong>を生成し、続けてOpenAI Image 2.0が<strong>サムネ画像3パターン</strong>を生成します。所要時間は約1〜3分です。<br />
...
function Step5({ result, setResult }: any) {
  ...
  const updateThumbnail = (index: number, updates: any) => {
    setResult((r: any) => ({
      ...r,
      thumbnails: r.thumbnails.map((t: any, i: number) => i === index ? { ...t, ...updates } : t)
    }));
  };

  return (
    <div style={styles.stepContent}>
      <SectionTitle title="生成結果" desc="タイトル案3つとサムネ画像3パターンが完成しました" />
      ...
      <div style={styles.patternsHeader}>
        <div>
          ...
          <p style={styles.patternsDesc}>生成した3パターン（1280×720）</p>
          {result._debug && (
            <p style={{
              fontSize: 11,
              color: result._debug.trainingImagesUsed > 0 ? '#0F8050' : '#A0411F',
              marginTop: 4,
              fontFamily: FONT_MONO,
            }}>
              🌟 学習画像: {result._debug.trainingImagesUsed}枚使用
              {result._debug.trainingImagesUsed > 0 &&
                ` (${result._debug.trainingImageNames.slice(0, 3).join(', ')}${result._debug.trainingImageNames.length > 3 ? '...' : ''})`}
            </p>
          )}
        </div>
      ...
      <div className="thumb-grid-3plus2">
        {result.thumbnails?.map((t: any, i: number) => (
          <ThumbnailImageCard
            key={i}
            data={t}
            index={i}
            onUpdate={(updates: any) => updateThumbnail(i, updates)}
          />
        ))}
      </div>
    </div>
...
function Pills({ options, value, onChange, multi, stack }: any) {
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, flexDirection: stack ? 'column' : 'row' }}>
      {options.map((o: any) => {
        // o が文字列ならそのまま、オブジェクトなら id/label/desc を取り出す
        const isObj = typeof o === 'object' && o !== null;
        const optId = isObj ? o.id : o;
        const optLabel = isObj ? o.label : o;
        const optDesc = isObj ? o.desc : null;
        const active = multi ? value.includes(optId) : value === optId;
        const handleClick = () => {
          if (multi) onChange(optId);
          else onChange(active ? '' : optId);
        };

        // 説明文付きの場合は2行表示のピル、なければ通常の1行ピル
        if (optDesc) {
          return (
            <button key={optId} onClick={handleClick}
              style={{
                ...styles.pill,
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', lineHeight: 1.2 }}>
                ...
                <span style={{ fontWeight: 600 }}>{optLabel}</span>
              </div>
              <div style={{
                fontSize: 10.5,
...
// ============================================================
// THUMBNAIL IMAGE CARD (with edit modal)
// ============================================================
function ThumbnailImageCard({ data, index, onUpdate }: any) {
  const [textCopied, setTextCopied] = useState(false);
  const [showEditModal, setShowEditModal] = useState(false);
  const Icon = getDirectionIcon(data.label);
  const t = data.thumbnailText || { badge: '', headline: '', mainCopy: '', subCopy: '', marker: '' };

  const copyText = () => {
    // 全テキスト要素を改行区切りで結合（空のものは除外）
    const parts = [t.badge, t.headline, t.mainCopy, t.subCopy].filter(Boolean);
      ...
      <div style={styles.thumbCard}>
        ...
        <div style={styles.thumbCardHeader}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0 }}>
            <div style={styles.thumbCardIcon}><Icon size={14} /></div>
            <div style={{ minWidth: 0, flex: 1 }}>
              <div style={styles.thumbCardNum}>PATTERN {String(index + 1).padStart(2, '0')}</div>
              <div style={{ ...styles.thumbCardLabel, fontSize: 11 }}>{data.label}</div>
            </div>
          </div>
        </div>
        ...
        {data.imageBase64 ? (
          ...
          <div style={{ padding: 24, background: '#FFEBEE', color: '#FF0000', textAlign: 'center', fontSize: 12, borderTop: '1px solid #E5E5E5' }}>
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 8 }}>
              <AlertCircle size={24} />
            </div>
            <div style={{ fontWeight: 700, marginBottom: 6 }}>画像生成に失敗しました</div>
            <div style={{ fontSize: 10, opacity: 0.8, lineHeight: 1.5, wordBreak: 'break-word' }}>{data.imageError}</div>
          </div>
        ) : (
          <div style={{ padding: 24, background: '#FAFAFA', textAlign: 'center', fontSize: 12, color: '#909090', borderTop: '1px solid #E5E5E5' }}>
            画像なし
          </div>
        )}
        ...
        {/* Action buttons row */}
        {data.imageBase64 && (
          <div style={{ padding: '8px 12px', display: 'flex', gap: 6, borderTop: `1px solid ${C.border}` }}>
        ...
        {/* Text section */}
        <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 6, borderTop: `1px solid ${C.border}` }}>
          <div style={{ ...styles.patternSectionLabel, fontSize: 10 }}>
            <Type size={11} />
            <span>サムネ文字案</span>
            <button onClick={copyText} style={{
              ...
              {textCopied ? <Check size={10} /> : <Copy size={10} />}
              <span>{textCopied ? 'コピー済' : 'コピー'}</span>
            </button>
          </div>
          <div style={{ ...styles.patternTextWrap, padding: 10 }}>
            {t.badge && (
              <div style={{
                ...
                {renderWithMarker(t.badge, t.marker)}
              </div>
            )}
            {t.headline && (
              <div style={{ ...styles.thumbSub, fontSize: 12, color: '#5A5A5A', marginBottom: 2 }}>
                {renderWithMarker(t.headline, t.marker)}
              </div>
            )}
            <div style={{ ...styles.thumbMain, fontSize: 18 }}>{renderWithMarker(t.mainCopy, t.marker)}</div>
            {t.subCopy && <div style={{ ...styles.thumbSub, fontSize: 13 }}>{renderWithMarker(t.subCopy, t.marker)}</div>}
            {t.marker && (
              <div style={{ ...styles.markerNote, fontSize: 10 }}>
...
// ============================================================
// EDIT MODAL - Region selection + AI re-generation
// ============================================================
function EditModal({ data, index, onClose, onUpdate }: any) {
  ...
  const baseImage = data.editedImageBase64 || data.imageBase64;
  ...
  const handleAddRegion = (region: any) => {
    setRegions([...regions, { ...region, id: `r-${Date.now()}` }]);
  };
  ...
  const applyEdits = async () => {
    if (regions.length === 0 && selectedPresets.length === 0 && !freeInstruction.trim()) {
      setEditError('範囲指定・プリセット・自由指示のいずれか1つ以上を入力してください');
      return;
    }
    ...
    try {
      ...
      let editPrompt = data.imagePrompt + '\n\n## ADDITIONAL EDIT INSTRUCTIONS\n';

      // Region-based instructions
      if (regions.length > 0) {
        editPrompt += buildRegionInstruction(regions);
      }
      ...
      if (selectedPresets.length > 0) {
        ...
        selectedPresets.forEach(id => {
          const preset = EDIT_PRESETS.find(p => p.id === id);
          if (preset) editPrompt += `- ${preset.prompt}\n`;
        });
      }

      // Free-text instruction
      if (freeInstruction.trim()) {
        editPrompt += `\n\n## USER FREE INSTRUCTION\n${freeInstruction.trim()}\n`;
      }
      ...
      if (!resp.ok) {
        const errJson = await resp.json().catch(() => ({}));
        throw new Error(errJson.error || `画像編集エラー (${resp.status})`);
      }

      const json = await resp.json();
      const newB64 = json.imageBase64;

      if (!newB64) throw new Error('画像データが返ってきませんでした');

      // Update with edited version
      onUpdate({
        editedImageBase64: newB64,
        imageBase64: newB64,
        originalImageBase64: data.originalImageBase64 || data.imageBase64,
      });

      onClose();
    } catch (e: any) {
      setEditError(e.message || '不明なエラー');
    } finally {
  ...
  const resetToOriginal = () => {
    if (!data.originalImageBase64) return;
    if (!confirm('編集前の画像に戻しますか？')) return;
    onUpdate({
      imageBase64: data.originalImageBase64,
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontSize: 11, fontFamily: FONT_MONO, color: C.red, fontWeight: 700, letterSpacing: 1 }}>
              PATTERN {String(index + 1).padStart(2, '0')} を編集
            </div>
        ...
        <div style={{ padding: 20, overflowY: 'auto', flex: 1 }}>
          ...
          {baseImage && (
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 11, color: C.textDim, marginBottom: 8, lineHeight: 1.5 }}>
                画像をマウスでドラッグすると範囲を選択できます。選択後、その範囲をどう変更するかを指定します。
              </div>
              <RegionSelector
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <textarea
              ...
              placeholder="例: もっと明るく / 背景を青に / 矢印を追加 など"
              style={{
                width: '100%',
                padding: 12,
                fontSize: 13,
...
function getDemoResult() {
  ...
  return {
    isDemo: true,
    titles: [
      { text: '新NISA 月3万｜住宅ローン返済中の会社員が老後不安を消した3ステップ', trigger: '知らないと損型', expectedCTR: '高' },
      { text: '【2026年版】38歳から始める新NISA｜プロが教える失敗しない買い方', trigger: '結論先出し型', expectedCTR: '中高' },
      { text: 'S&P500だけじゃ危険？ 9割が知らない新NISAの落とし穴と対策5選', trigger: '問題提起型', expectedCTR: '高' },
    ],
    thumbnails: thumbs.map(t => ({
      label: t.label,
      thumbnailText: { badge: '', headline: '', mainCopy: t.main, subCopy: t.sub, marker: t.marker },
